"""Offline trainer: Dueling Double DQN — Phase 3 v4 (moving + blinking box + wall).

Curriculum:
  difficulty=0 for ep_switch1 episodes  → build base push policy (no wall)
  difficulty=2 for ep_switch2 episodes  → adapt to blinking box (with wall)
  difficulty=3 for remaining episodes   → moving + blinking box (with wall)

Key improvements over v3:
  - Much stronger sensor bias (10.0 for IR, 5.0 for forward, 3.0 for side)
  - Biased exploration: FW 65% of time so agent covers arena and finds box
  - Rotate then FW recovery (12 rotate + 3 forward steps)
  - push_repeat=15
  - Hidden 256

Example:
  python train_d3qn_p3_v4.py --obelix_py ./obelix.py --out weights_p3_v4.pth --wall_obstacles
"""
...
ROT_IDX       = {0, 1, 3, 4}
EXPLORE_PROBS = np.array([0.05, 0.10, 0.65, 0.10, 0.05], dtype=np.float32)
...
def greedy_action(qs: np.ndarray, obs: np.ndarray, last_a: int,
    ...
    """Greedy: strong sensor bias + rotate-then-FW recovery + anti-spin."""

    # wall collision — start 15-step recovery
    if obs[17] > 0:
        new_stuck    = stuck + 1
        new_recovery = 15
        if (new_stuck // 3) % 2 == 0:
    ...
    # countdown recovery — last 3 steps force FW to clear wall
    if recovery > 0:
        new_recovery = recovery - 1
        if new_recovery < 3:
            return FW_IDX, 0, stuck, new_recovery
        if (stuck // 3) % 2 == 0:
    ...
    # strong sensor bias — forces agent toward box when detected
    if obs[16] > 0:             # IR sensor — box directly ahead very close
        q[FW_IDX] += 10.0
    elif any(obs[4:12] > 0):    # forward sonars — box ahead
        q[FW_IDX] += 5.0
    elif any(obs[0:4] > 0):     # left sonars — box to left
        q[1]      += 3.0        # L22
    elif any(obs[12:16] > 0):   # right sonars — box to right
        q[3]      += 3.0        # R22

    best = int(np.argmax(q))

    # anti-spin — 2 same rotations in a row → force FW
...
def main():
    ...
    ap.add_argument("--out",             type=str,   default="weights_p3_v4.pth")
    ...
    # d0 without wall — learn navigation without relying on wall bouncing
    env_d0 = OBELIX(
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if np.random.rand() < eps:
                # biased exploration — FW 65% to cover arena and find box faster
