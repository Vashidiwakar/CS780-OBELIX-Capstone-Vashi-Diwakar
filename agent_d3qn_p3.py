"""Dueling Double DQN agent for OBELIX — Phase 3.
Evaluation-only: loads pretrained weights from weights.pth.
Improvements:
  - Stuck recovery with countdown timer (8 steps after wall collision)
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    # if wall collision, start 8-step recovery burst
    if obs[17] > 0:
        _stuck_count    += 1
        _recovery_steps  = 8
        _repeat_count    = 0
