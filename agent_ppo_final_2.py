...
Key improvements over Phase 2 agent:
  - L22/R22 instead of L45/R45 for dark rotation
    Sensors fire for 2x longer per step — agent doesn't overshoot box
  - Sensor persistence: keeps moving toward box for 3 steps after sensor fires
    Solves the 1-step sensor flash problem discovered in debugging
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    # reset dark_dir every 200 steps to vary sweep direction
    ...
    if obs[16] == 1:
        _dark_steps = 0
        _fw_persist = 5   # persist FW for 5 steps
        return "FW"

    # Priority 3 — sensor persistence (box detected recently)
    if _fw_persist > 0:
        _fw_persist -= 1
        return "FW"
    ...
    # Priority 5 — left sonars active, turn left then persist
    if any(obs[0:4] == 1):
        _dark_steps = 0
        _fw_persist = 2
        return "L22"

    # Priority 6 — right sonars active, turn right then persist
    if any(obs[12:16] == 1):
        _dark_steps = 0
        _fw_persist = 2
        return "R22"

    # Priority 7 — all sensors dark, fine-grain rotation sweep
    _dark_steps += 1
