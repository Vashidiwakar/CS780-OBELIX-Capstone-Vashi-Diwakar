"""Offline trainer: Dueling Double DQN — Phase 2 (blinking box + wall).

Curriculum: difficulty=0 for ep_switch episodes, then difficulty=2.
Improvements:
  - Successful push transitions stored 6x in replay buffer
  - Longer exploration (eps_decay_steps=800k, eps_end=0.15)
  - Longer d0 phase (ep_switch=1200)
  - gc.collect() every episode to manage memory

Example:
  python train_d3qn_phase2.py --obelix_py ./obelix.py --out weights_phase2.pth --wall_obstacles
"""
...
def main():
    ...
    ap.add_argument("--push_repeat",     type=int,   default=6)   # times to store push transitions
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            # +100 = attached to box, +2000 = delivered to boundary
            if r >= 100:
