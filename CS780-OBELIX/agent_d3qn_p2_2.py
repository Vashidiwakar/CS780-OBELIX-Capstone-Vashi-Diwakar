"""Dueling Double DQN agent for OBELIX — Phase 2.
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    if obs[16] > 0:             # IR sensor fires — box very close directly ahead
        q[2] += 2.0
    elif any(obs[4:12] > 0):    # forward sonars active — box detected ahead
        q[2] += 0.5

    best = int(np.argmax(q))

    # anti-spin: if same rotation chosen 4 times in a row, force FW
    if _last_action is not None and best == _last_action and best in (0, 1, 3, 4):
        _repeat_count += 1
        if _repeat_count >= 4:
            best = 2  # force FW
            _repeat_count = 0
