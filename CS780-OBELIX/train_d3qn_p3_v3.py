"""Offline trainer: Dueling Double DQN — Phase 3 v3 (moving + blinking box + wall).
...
Key improvements over v2:
  - Improved stuck recovery: rotate 12 steps then force FW 3 steps to move away from wall
  - Full sensor steering (IR, forward, left, right sonars)
  - Anti-spin threshold 2 repeats
  - Recovery steps 15 total (12 rotate + 3 forward)
  - push_repeat=15
  - Hidden size 256
  - d0 trained without wall for better generalisation

Example:
  python train_d3qn_p3_v3.py --obelix_py ./obelix.py --out weights_p3_v3.pth --wall_obstacles
"""
...
def greedy_action(qs: np.ndarray, obs: np.ndarray, last_a: int,
    ...
    """Greedy action: rotate away from wall then FW to clear, sensor steering, anti-spin."""

    # if wall collision detected, start 15-step recovery
    if obs[17] > 0:
        new_stuck    = stuck + 1
        new_recovery = 15
        if (new_stuck // 3) % 2 == 0:
...
def main():
    ...
    ap.add_argument("--out",             type=str,   default="weights_p3_v3.pth")
    ...
    # d0 without wall — agent learns to navigate without relying on wall bouncing
    env_d0 = OBELIX(
