"""Offline trainer: Dueling Double DQN — Phase 2 v5 (blinking box + wall).

Curriculum: difficulty=0 for ep_switch episodes, then difficulty=2.
Key improvements over v4:
  - Improved stuck recovery using countdown timer (8 recovery steps)
    obs[17] resets to 0 after one step even if still against wall,
    so we count down 8 forced rotation steps after each collision
  - Forward bias during greedy action selection
  - Anti-spin during greedy selection
...
Example:
  python train_d3qn_p2_5.py --obelix_py ./obelix.py --out weights_p2_5.pth --wall_obstacles
"""
...
def greedy_action(qs: np.ndarray, obs: np.ndarray, last_a: int,
    ...
    """Greedy action with countdown stuck recovery, forward bias and anti-spin."""

    # if wall collision detected, start 8-step recovery burst
    if obs[17] > 0:
        new_stuck    = stuck + 1
        new_recovery = 8
        if (new_stuck // 3) % 2 == 0:
    ...
    # anti-spin — same rotation 8 times in a row → force FW
...
def main():
    ...
    ap.add_argument("--out",             type=str,   default="weights_p2_5.pth")
