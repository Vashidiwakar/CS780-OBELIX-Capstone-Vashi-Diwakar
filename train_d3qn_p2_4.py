"""Offline trainer: Dueling Double DQN — Phase 2 v4 (blinking box + wall).

Curriculum: difficulty=0 for ep_switch episodes, then difficulty=2.
Key improvements:
  - Stuck recovery during greedy selection (alternates L45/R45 on wall collision)
  - Forward bias during greedy action selection
  - Anti-spin during greedy selection (breaks rotation loops)
  - Successful push/delivery transitions stored 6x in replay
  - Longer exploration (eps_decay_steps=800k, eps_end=0.15)
  - Longer d0 phase (ep_switch=1200)
  - gc.collect() every episode

Example:
  python train_d3qn_p2_3.py --obelix_py ./obelix.py --out weights_p2_3.pth --wall_obstacles
"""
...
def greedy_action(qs: np.ndarray, obs: np.ndarray, last_a: int,
                  repeat: int, stuck: int):
    """Greedy action with stuck recovery, forward bias and anti-spin."""

    # stuck recovery — obs[17] is wall collision flag
    if obs[17] > 0:
        new_stuck = stuck + 1
        # alternate L45/R45 every 3 stuck steps to escape corners
        if (new_stuck // 3) % 2 == 0:
            return 0, 0, new_stuck   # L45
        else:
            return 4, 0, new_stuck   # R45

    q = qs.copy()

    # forward bias when box detected ahead
    if obs[16] > 0:            # IR sensor fires
        q[FW_IDX] += 2.0
    elif any(obs[4:12] > 0):   # forward sonars fire
        q[FW_IDX] += 0.5

    best = int(np.argmax(q))

    # anti-spin — same rotation 4 times in a row → force FW
    if last_a is not None and best == last_a and best in ROT_IDX:
        if repeat >= 4:
            return FW_IDX, 0, 0
        return best, repeat + 1, 0
...
def main():
    ...
    ap.add_argument("--out",             type=str,   default="weights_p2_3.pth")
    ap.add_argument("--episodes",        type=int,   default=3000)
    ap.add_argument("--max_steps",       type=int,   default=1000)
    ap.add_argument("--ep_switch",       type=int,   default=1200)
    ...
    ap.add_argument("--eps_decay_steps", type=int,   default=800_000)
    ap.add_argument("--push_repeat",     type=int,   default=6)
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if np.random.rand() < eps:
                ...
                a, repeat, stuck = greedy_action(qs, s, last_a, repeat, stuck)
                last_a = a
            ...
            # r >= 100 means attached to box (+100) or delivered (+2000)
            if r >= 100:
