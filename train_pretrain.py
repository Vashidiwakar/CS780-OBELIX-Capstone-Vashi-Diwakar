"""Stage 1: Pretrain Dueling DDQN on difficulty=0, NO walls.

Goal: get agent reliably pushing box to boundary before adding complexity.
Run this first, then run train_finetune.py with the saved weights.

Example:
  python train_pretrain.py --obelix_py ./obelix.py --out weights_pretrained.pth
"""
...
def main():
    ...
    ap.add_argument("--out",             type=str,   default="weights_pretrained.pth")
    ...
    # NO walls, difficulty=0 only — simplest possible environment
    env = OBELIX(
    ...
    for ep in range(args.episodes):
        ...
        if (ep + 1) % 50 == 0:
            print(f"Episode {ep+1}/{args.episodes} [pretrain] return={ep_ret:.1f} eps={eps_by_step(steps):.3f} replay={len(replay)}")
