"""Offline trainer: Dueling Double DQN — Phase 2 v3 (blinking box + wall).

Curriculum: difficulty=0 for ep_switch episodes, then difficulty=2.
Key improvements over v2:
  - Forward bias during greedy action selection (agent learns to move forward)
  - Anti-spin during greedy selection (breaks rotation loops in training)
  - Successful push/delivery transitions stored 6x in replay
  - Longer exploration (eps_decay_steps=800k, eps_end=0.15)
  - Longer d0 phase (ep_switch=1200) to build stronger base policy
...
ROT_IDX = {0, 1, 3, 4}          # L45, L22, R22, R45
...
def greedy_action(qs: np.ndarray, obs: np.ndarray, last_a: int, repeat: int):
    """Greedy action selection with forward bias and anti-spin."""
    q = qs.copy()

    # forward bias — boost FW when box detected ahead
    if obs[16] > 0:           # IR sensor — box very close directly ahead
        q[FW_IDX] += 2.0
    elif any(obs[4:12] > 0):  # forward sonars — box detected ahead
        q[FW_IDX] += 0.5

    best = int(np.argmax(q))

    # anti-spin — if same rotation 4 times in a row, force FW
    if last_a is not None and best == last_a and best in ROT_IDX:
        if repeat >= 4:
            return FW_IDX, 0
        return best, repeat + 1
...
def main():
    ...
    # create both envs once outside loop
    env_d0 = OBELIX(
    ...
    for ep in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if np.random.rand() < eps:
                # random exploration — no bias needed
                ...
                a, repeat = greedy_action(qs, s, last_a, repeat)
                last_a = a
            ...
            # r >= 100 means robot attached to box (+100) or delivered (+2000)
            if r >= 100:
